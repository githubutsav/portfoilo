  ...
  useEffect(() => {
    ...
    const updateVisibleTags = () => {
      ...
      for (let i = 0; i < tags.length; i++) {
        const itemWidth = items[i]?.offsetWidth || 60
    ...
    <ul ref={containerRef} className="flex items-center gap-2 overflow-hidden flex-nowrap shrink min-w-0 w-full">
      {tags.map((tag, idx) => (
        <li
          ...
          data-tag-item
          className={cn(
            'font-mono text-xs text-muted-foreground shrink-0 whitespace-nowrap',
      ...
      {hiddenCount > 0 && (
        <li className="rounded bg-primary/10 px-1.5 py-0.5 font-mono text-[11px] font-medium text-primary shrink-0 whitespace-nowrap">
          +{hiddenCount}
        </li>
      )}
    </ul>
...
export function Projects() {
  ...
  return (
    <section id="work" className="scroll-mt-0 border-t border-border py-20 md:py-28">
        ...
        <SectionHeading
          ...
          actions={
            <>
              <Link
                href="/projects"
                className="inline-flex items-center gap-1 font-mono text-xs text-muted-foreground transition-colors hover:text-primary whitespace-nowrap"
              >
                <span>view all <span className="hidden sm:inline">({projectsData.length})</span></span>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="hidden sm:inline-flex items-center gap-1 font-mono text-xs text-muted-foreground transition-colors hover:text-primary whitespace-nowrap"
              >
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-2">
          {visibleProjects.map((project) => (
            <article
              ...
              className="group flex w-full cursor-pointer flex-col justify-between overflow-hidden rounded-xl border border-border bg-card p-5 sm:p-6 transition-all hover:border-primary/50 hover:shadow-xl hover:shadow-primary/5"
            >
